...
//        frame depth = aligned_set.get_depth_frame();
        Mat color_mat = imread(rgb_file[i]);
//
...
//        auto near = frame_to_mat(bw_depth);
        Mat near = imread(depth_file[i], IMREAD_ANYDEPTH);
//        cvtColor(near, near, COLOR_BGR2GRAY);
        near.convertTo(near,CV_8UC1,1/256);
        ...
        Mat far = imread(depth_file[i], IMREAD_ANYDEPTH);
//        cvtColor(far, far, COLOR_BGR2GRAY);
        far.convertTo(far,CV_8UC1,1/256);
        far.setTo(255, far == 0); // Note: 0
        // value does not indicate pixel near the camera, and requires special attention
